...
RUN apt-get -y update && apt-get -y install git make gcc liblua5.3-0 liblua5.3-dev \
  ...
  && make -j $(nproc) \
    ...
    TARGET=linux-glibc \
    CPU=generic \
    $(if [ "$SSLLIB" = "QuicTLS" ] || [ "$SSLLIB" = "OldQuicTLS" ]; then echo USE_OPENSSL=1; elif [ "$SSLLIB" = "LibreSSL" ]; then echo USE_OPENSSL=1; elif [ "$SSLLIB" = "AWS-LC" ]; then echo USE_OPENSSL_AWSLC=1; else echo "not supported SSLLIB"; exit 1; fi) \
    USE_QUIC=1 \
